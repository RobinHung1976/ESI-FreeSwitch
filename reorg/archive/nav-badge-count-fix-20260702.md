...
2. **Nav badge**（`updateNavBadge()`）：每次都獨立呼叫 `/api/calls`，直接採用 FreeSWITCH `show calls as json` 的 `row_count`，未去重，且**只靠 `CHANNEL_*` WebSocket 事件觸發更新**，沒有任何校正機制。一旦 WebSocket 斷線期間漏接一次 `CHANNEL_DESTROY` 事件，計數就會永久卡在舊值，直到手動重新整理頁面（重跑 `init.js` 的 `updateNavBadge()`）。
...
2. **`common.js`** — `initWebSocket()` 的 `ws.onopen` 內新增 `_refreshMonitorCallsTable()`，確保首次連線與每次斷線重連（`onclose` → `setTimeout(initWebSocket, 5000)`）都會強制對齊 FreeSWITCH 實際通話狀態，避免漏接事件造成飄移：
```javascript
ws.onopen = () => {
...
**排查方式**：
```bash
# fs_cli 需帶正確連線參數（非預設 8021/ClueCon），見 settings.json
...
**驗證方式**：無通話時左側 badge 與 header badge（`LIVE · 0 通話中`）皆應顯示 0；手動斷開/重連 WebSocket（或重啟 `fs-dashboard.service`）後兩者仍應維持一致，不再需要手動重新整理頁面才能校正。
