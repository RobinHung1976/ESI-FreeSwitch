...
- `requirements.txt` 的 `websockets` 套件**未鎖版本**，環境重裝/重建（例如自動備份腳本的 `pip install`）時抓到最新的 **v14+**。
...
- 由於是**每個連線各自的 handler 崩潰**，並不會讓 `fs-dashboard.service` 這個 process 掛掉，所以 `systemctl status` 顯示一切正常，只有前端連線狀態燈是紅燈，容易誤判為網路或設定問題。
