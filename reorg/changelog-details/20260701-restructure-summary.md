# FreeSwitch Dashboard 專案重整總結 — 2026-07-01

> 原始來源：`fs-dashboard-restructure-summary-20260701.md`
> 範圍：Dialplan UI 整合、側邊欄優化、前後端程式碼模組化重整、正式部署

## 一、起因

1. Dialplan 相關功能散落在三個地方：sidebar 3 個獨立項目（路由規則／系統內建／自定義）+ 設定頁裡又有一個容易混淆的「Dialplan 設定」子選單
2. 側邊欄項目越來越多，空間不夠用
...
| 設定頁「Dialplan 設定」分組 | 整組移除。「Context 清單」直接砍掉；「檔案路徑」拉平成獨立項目；「重載指令」搬到 ESL 終端機頁面變成常用指令快捷區 |
| Sidebar 空間 | 監控／管理／系統三個分類做成可收合 accordion，狀態存 `localStorage` |

## 三、程式碼模組化重整（第二階段）

**問題**：`server.py` 3,470 行、約 70 個 API 端點全部寫在同一支檔案；`index.html` 10,634 行、293 個函式全部塞在同一個 `<script>` 標籤裡；資料夾裡混了一份意外留下的複本檔案（`dialplan_routes - 複製.py`）。

**清理**：刪除 `dialplan_routes - 複製.py`（確認為意外複本）；`internal.xml`/`ivr_runner.lua` 維持原狀不動（屬於 FreeSwitch 本身，非 Dashboard app）。
...
core/
  ...
  ├─ esl_client.py / ws_manager.py / backup_manager.py / dialplan_common.py
...
static/
  ...
  css/style.css
  js/
    common.js            後端連線設定、WebSocket、API 工具（最先載入）
...
**驗證方式（自動化比對，非肉眼檢查）**：
- 後端：拆分前後兩版都實際 import 起來、用 `TestClient` 打 `/openapi.json`，逐一比對所有路徑與 HTTP method → **74 條路由，完全一致**
- 前端：把拆分前後所有頂層 `function`/`const`/`let` 宣告名稱排序後 diff → **297 個函式、107 個變數，完全一致**
- 順手發現並修正：`requirements.txt` 原本漏列 `python-multipart`、`lxml`
...
| 問題 | 原因 | 修正方式 |
|---|---|---|
| `Uncaught ReferenceError: renderOverview is not defined`、`Cannot access 'pages' before initialization` | `pages` 物件寫死引用各頁面的 render 函式，但被誤放進**最先載入**的 `common.js`；此時其他頁面的 render 函式都還沒定義 | 把 `pages` 物件、`switchPage`、`refreshData` 移到**最後載入**的 `init.js`，確保所有頁面函式都已載入完成才組裝 `pages` |
...
## 六、後續維護指引（已併入 `PROJECT-OVERVIEW.md` 開發慣例章節）
...
- 跨 router 共用邏輯：兩個 router 互相呼叫是正常的，但避免「互相 import」造成循環依賴；真的要共用的常數/狀態放進 `core/constants.py` 或 `core/state.py`
- `pages` 物件的教訓：任何直接寫死引用其他檔案函式的物件/陣列，一定要放在「所有依賴的函式都已定義」之後再宣告
