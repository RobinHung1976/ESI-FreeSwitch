...
> 原始來源：`recording-UI-scroll-player-fix-20260702.md`
...
**根本原因**：`wrap.innerHTML` 同時包含 `<table>` 與播放器 `<div>`，兩者綁在同一個重繪區塊裡。

**修復**（`recordings.js`）：播放器 HTML 搬到 `renderRecordings()` 的固定骨架中（僅建立一次），位置在篩選列下方、表格上方；`loadRecordings()` 的 `wrap.innerHTML` 只重繪 `<table>`，不再包含播放器；`_recPlay()` 移除顯示切換，播放器元素固定存在，只更新 `audio.src` 與標籤文字。
...
**現象**：套用修改後瀏覽器 console 出現 `Uncaught SyntaxError: Unexpected token '<'`、`Cannot access 'pages' before initialization`。

**根本原因**：`loadRecordings()` 內新版樣板字串已在 `</table>\`;` 結尾，但舊版播放器區塊沒刪乾淨，殘留在樣板字串外面變成裸露 HTML，導致整支 `recordings.js` parse 失敗，連帶讓 `init.js` 抓不到 `renderRecordings`、`pages` 也未初始化完成。

**修復**：刪除殘留的舊播放器區塊，確認 `</table>\`;` 後直接接 `if (pager) {`。

**提醒**：改樣板字串（template literal）時，若舊程式碼原本橫跨多個反引號區塊，刪減/搬移後務必確認反引號收尾位置正確；`Unexpected token '<'` 幾乎都是「HTML 漏在字串外」的訊號。
