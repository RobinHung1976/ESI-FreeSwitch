...
13. **分機 Context 表單缺少「儲存前檢查 context 是否真的存在」的防呆**：`feature-extensions.md` 現有的「保留原值供選擇並標示警語」只涵蓋「context 資料夾事後被刪除」的情境，若一開始就選/填了一個從未存在過的 context（2026-08-11 查證 `USER_NOT_REGISTERED` 過程中意外發現分機 1126 被設成不存在的 `TC-ACSBC`，導致該分機完全無法撥出，直接 `NO_ROUTE_DESTINATION`），目前沒有任何防呆機制擋下。是否要在儲存時比對 `/api/dialplan/contexts` 清單，使用者表示之後再討論，非本次處理範圍，見 `changelog-details/20260811-user-not-registered-verification.md` 第四節。
