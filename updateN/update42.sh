#!/bin/bash
# update42.sh — 接續完成 update40.sh（update41.sh 因前置驗證邏輯誤判而中止，未寫入任何檔案）
#
...
# 一次全部 commit。
#
# update41.sh 的前置驗證誤用「檔案裡有沒有出現 token= pattern」判斷是否已改過，
# 結果被 logs.js 裡 update39.sh 已修好的 SSE 那行（本來就含 token=）誤觸發而中止，
# 未寫入任何檔案，安全。本版改成針對每個目標字串精確比對是否仍為原始狀態。
...
grep -qF 'src="${API_BASE}/api/sounds/stream?path=${encodeURIComponent(s.path)}"' static/js/ivr.js \
  || { echo "❌ static/js/ivr.js 目標字串不是原始狀態，中止避免重複修改" >&2; exit 1; }

grep -qF 'audio.src = `${API_BASE}/api/sounds/stream?path=${encodeURIComponent(path)}`;' static/js/settings-vars.js \
  || { echo "❌ static/js/settings-vars.js 音檔試聽目標字串不是原始狀態，中止" >&2; exit 1; }
grep -qF 'a.href  = `${API_BASE}/api/backup/download?filename=${encodeURIComponent(filename)}`;' static/js/settings-vars.js \
  || { echo "❌ static/js/settings-vars.js 備份下載目標字串不是原始狀態，中止" >&2; exit 1; }

grep -qF 'player.src = `${API_BASE}/api/sounds/stream?path=${encodeURIComponent(path)}`;' static/js/sounds.js \
  || { echo "❌ static/js/sounds.js 目標字串不是原始狀態，中止" >&2; exit 1; }

grep -qF 'a.href     = `${API_BASE}/api/logs/download?date=${_histDate}`;' static/js/logs.js \
  || { echo "❌ static/js/logs.js download 目標字串不是原始狀態，中止" >&2; exit 1; }

grep -qF 'const url = `${API_BASE}/api/download?path=${encodeURIComponent(path)}`;' static/js/backup.js \
  || { echo "❌ static/js/backup.js 目標字串不是原始狀態，中止" >&2; exit 1; }

grep -qF 'const streamUrl = `${API_BASE}/api/recordings/stream?path=${encodeURIComponent(f.path)}`;' static/js/recordings.js \
  || { echo "❌ static/js/recordings.js 目標字串不是原始狀態，中止" >&2; exit 1; }
